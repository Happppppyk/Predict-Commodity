"""
역할: master_daily + features.yaml 지정 버전(v1, v1_stationary 등) XGBoost 이진 분류(target_updown_t7).
레이어: 예측 모델 (3) — 시계열 순서를 지키는 train/test 분리 및 TreeSHAP 해석.
"""
...
# `src`를 path에 넣어 `utils.plot_style` 로드 (스크립트 직접 실행 호환)
...
matplotlib.use("Agg")  # GUI 없는 환경에서 SHAP 플롯 저장
...
def load_features(
    ...
    *,
    ...
    if tcol == "target_updown_t28":
        tail_n = min(28, len(raw))
        tail_nan = int(raw[tcol].tail(tail_n).isna().sum()) if tail_n > 0 else 0
        print(
            f"[load_features] t28 tail NaN 점검: 마지막 {tail_n}행 중 NaN {tail_nan}행"
        )
...
def run_tuned_pipeline(
    ...
    *,
    ...
    model_path = MODELS_DIR / f"xgb_{version}_tuned.json"
    shap_path = shap_summary_path or (NOTEBOOKS_DIR / f"shap_summary_{version}_tuned.png")
...
def walk_forward_validation(
    ...
    *,
    ...
    - 하이퍼파라미터는 `WALK_FORWARD_FIXED_PARAMS` 고정
    """
    db = Path(db_path) if db_path else DEFAULT_DB_PATH
    if not db.is_file():
        raise FileNotFoundError(f"DB 파일이 없습니다: {db.resolve()}")
...
def run_walk_forward(
    ...
    *,
    ...
    results_df = walk_forward_validation(
        db_path, version=version, target=target, config_path=config_path
    )
    NOTEBOOKS_DIR.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Optuna 시도 횟수 (--tuned 일 때만 사용)",
    )
    parser.add_argument(
        "--walkforward",
        action="store_true",
